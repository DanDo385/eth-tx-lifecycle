...
// sandwichWorkers controls how many receipt fetches run in parallel.
// Default 10 gives a ~10x speedup over sequential while not overwhelming the RPC node.
var sandwichWorkers = func() int {
	s := envOr("SANDWICH_WORKERS", "10")
	n, err := strconv.Atoi(s)
	if err != nil || n < 1 {
		return 10
	}
...
func collectSwaps(b *block) ([]swapEvent, error) {
	...
	// Pre-allocate result slots indexed by transaction position to preserve ordering.
	// Each slot holds the swaps found in that transaction (nil if none or fetch failed).
	results := make([][]swapEvent, maxN)

	// Bounded worker pool using a semaphore channel
