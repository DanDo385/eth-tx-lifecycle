...
export function weiToEth(hexWei: string): string {
  ...
  try {
    ...
    // For extremely small values (< 0.000001 ETH), show as 0 ETH
...
// Convert block number to decimal number (handles both hex "0x..." and decimal strings)
// Relay APIs return block_number as decimal strings, while RPC APIs return hex
export function blockNumberToNumber(blockNumber: string | number): number {
  if (typeof blockNumber === 'number') return blockNumber;
  if (!blockNumber || blockNumber === '0x0' || blockNumber === '0x' || blockNumber === '0') return 0;
  try {
