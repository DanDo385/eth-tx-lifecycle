...
export default function EstimateMethodologyCard({
  ...
  return (
    <section className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 md:p-5">
      <h3 className="text-xl font-semibold text-white">Estimated pre-PBS vs post-PBS comparison</h3>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        <div className="rounded-lg border border-white/15 bg-black/25 p-3">
          <div className="text-xs text-white/60">Observed current MEV-tagged block share</div>
          <div className="mt-1 text-2xl font-bold text-white">{mevTaggedSharePct.toFixed(1)}%</div>
          <div className="text-xs text-emerald-300">Confidence: High (directly from current headers)</div>
        </div>
        <div className="rounded-lg border border-white/15 bg-black/25 p-3">
          <div className="text-xs text-white/60">Observed avg builder payment (current)</div>
          <div className="mt-1 text-2xl font-bold text-white">{avgBuilderPaymentEth.toFixed(4)} ETH</div>
          <div className="text-xs text-emerald-300">Confidence: High (live header enrichment)</div>
        </div>
        <div className="rounded-lg border border-white/15 bg-black/25 p-3">
          <div className="text-xs text-white/60">Observed MEV event rate (scan sample)</div>
          <div className="mt-1 text-2xl font-bold text-white">{(mevEventsPerScannedTx * 100).toFixed(2)}%</div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="rounded-lg border border-red-400/30 bg-red-500/10 p-3">
          ...
          <div className="mt-1 text-xl font-semibold text-white">{(estimatedPrePbsMevEventRate * 100).toFixed(2)}%</div>
        ...
        <div className="rounded-lg border border-red-400/30 bg-red-500/10 p-3">
          ...
          <div className="mt-1 text-xl font-semibold text-white">{estimatedPrePbsBuilderEquivalent.toFixed(4)} ETH</div>
          <p className="mt-1 text-xs text-red-100">
      ...
      <div className="mt-4 rounded border border-white/15 bg-black/30 p-3 text-xs text-white/65">
