...
import { weiToEth, hexToGwei, hexToNumber, shortenHash, formatNumber, slotToEpoch, blockNumberToNumber } from '../utils/format';
    ...
    <div className="space-y-4 text-sm">
      ...
      <div className="border-l-4 border-blue-500 pl-4">
        ...
        <div className="space-y-1 text-fg/80">
            ...
            <span className="font-medium">
              {isPending ? (
                ...
                <span className="text-green-400">
                  ✅ Confirmed in Block {blockNumberToNumber(inclusion?.block_number || '0x0').toLocaleString()}
                </span>
              )}
            </span>
          </div>
          {!isPending && inclusion?.timestamp && (
            <div className="flex justify-between">
              <span className="text-fg/60">Block Time:</span>
              <span>{new Date(hexToNumber(inclusion.timestamp) * 1000).toLocaleString()}</span>
            </div>
          )}
          <div className="flex justify-between">
            <span className="text-fg/60">From:</span>
            <span className="font-mono text-xs">{data.from}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-fg/60">To:</span>
            <span className="font-mono text-xs">{data.to || '(Contract Creation)'}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-fg/60">Value:</span>
            <span className="font-medium">
              {(() => {
                const valueEth = weiToEth(economics?.value || '0x0');
                // If value is 0 or very small, show it clearly
                if (valueEth === '0' || parseFloat(valueEth) < 0.000001) {
                  return '0 ETH';
                }
                return `${valueEth} ETH`;
              })()}
          ...
          {decoded?.action_type === 'swap' && decoded.details?.swap_from_amount_formatted && (
            <>
              <div className="border-t border-line/10 my-2 pt-2">
                <div className="text-fg/60 text-xs mb-1">Swap Details:</div>
              </div>
              <div className="flex justify-between">
                <span className="text-fg/60">From:</span>
              ...
              {decoded.details.price_per_token && (
                <div className="flex justify-between">
                  <span className="text-fg/60">Exchange Rate:</span>
          ...
          {decoded?.action_type === 'transfer' && decoded.details?.amount_wei && (
            <>
              <div className="border-t border-line/10 my-2 pt-2">
                <div className="text-fg/60 text-xs mb-1">Transfer Details:</div>
              </div>
              <div className="flex justify-between">
                <span className="text-fg/60">Amount:</span>
                <span className="font-medium">{weiToEth(decoded.details.amount_wei as string)} tokens</span>
              </div>
              {decoded.details.recipient && (
                <div className="flex justify-between">
                  <span className="text-fg/60">Recipient:</span>
          ...
          {decoded?.action_type === 'approve' && decoded.details?.amount_wei && (
            <>
              <div className="border-t border-line/10 my-2 pt-2">
                <div className="text-fg/60 text-xs mb-1">Approval Details:</div>
              </div>
              {decoded.details.spender && (
                <div className="flex justify-between">
                  <span className="text-fg/60">Spender:</span>
          ...
          {(decoded?.action_type === 'deposit' || decoded?.action_type === 'withdraw') && (
            <>
              <div className="border-t border-line/10 my-2 pt-2">
                <div className="text-fg/60 text-xs mb-1">{decoded.action_type === 'deposit' ? 'Deposit' : 'Withdraw'} Details:</div>
              </div>
              ...
              {decoded.details?.eth_amount && (
                <div className="flex justify-between">
                  <span className="text-fg/60">ETH Amount:</span>
          ...
          {decoded?.action_type === 'mint' && (
            <>
              <div className="border-t border-line/10 my-2 pt-2">
                <div className="text-fg/60 text-xs mb-1">Mint Details:</div>
              </div>
          ...
          {decoded?.action_type === 'claim' && decoded.details?.claimed_amount && (
            <>
              <div className="border-t border-line/10 my-2 pt-2">
                <div className="text-fg/60 text-xs mb-1">Claim Details:</div>
              </div>
              <div className="flex justify-between">
                <span className="text-fg/60">Claimed:</span>
          ...
          {decoded?.action_type === 'execute' && decoded.details?.target && (
            <>
              <div className="border-t border-line/10 my-2 pt-2">
                <div className="text-fg/60 text-xs mb-1">Execute Details:</div>
              </div>
              <div className="flex justify-between">
                <span className="text-fg/60">Target:</span>
                <span className="font-mono text-xs">{shortenHash(decoded.details.target as string)}</span>
              </div>
            </>
          )}

          {decoded?.action_type === 'contract_call' && (
            <>
              <div className="border-t border-line/10 my-2 pt-2">
                <div className="text-fg/60 text-xs mb-1">Contract Call Details:</div>
              </div>
              {decoded.details?.method_selector && (
                <div className="flex justify-between">
                  <span className="text-fg/60">Method Selector:</span>
                  <span className="font-mono text-xs text-purple-400">{decoded.details.method_selector as string}</span>
                </div>
              )}
              {decoded.contract_type && (
                <div className="flex justify-between">
                  <span className="text-fg/60">Contract:</span>
                  <span className="text-fg">{decoded.contract_type}</span>
                </div>
              )}
            </>
          )}
      ...
      {decoded && (
        <div className="border-l-4 border-indigo-500 pl-4">
          ...
          <div className="space-y-2 text-fg/80">
            <div className="flex justify-between items-start">
              <span className="text-fg/60">Action:</span>
            ...
            {decoded.method_name && (
              <div className="flex justify-between items-start">
                <span className="text-fg/60">Method:</span>
            ...
            {decoded.details?.transfers && Array.isArray(decoded.details.transfers) && (
              <div className="mt-3">
                <div className="text-fg/60 text-xs mb-2">Token Transfers ({decoded.details.transfer_count}):</div>
                <div className="space-y-2">
                  {(decoded.details.transfers as any[]).map((transfer: any, idx: number) => (
                    <div key={idx} className="bg-fill-subtle/5 rounded p-2 text-xs">
                      <div className="flex justify-between">
                        <span className="text-fg/60">Token:</span>
      ...
      <div className="border-l-4 border-green-500 pl-4">
        ...
        <div className="space-y-1 text-fg/80">
          <div className="flex justify-between">
            <span className="text-fg/60">Gas Limit:</span>
            <span>{formatNumber(hexToNumber(economics?.gas_limit || '0x0'))} gas</span>
          </div>
          {economics?.gas_used && (
            <div className="flex justify-between">
              <span className="text-fg/60">Gas Used:</span>
      ...
      {/* Execution Inclusion Section */}
      {!isPending && inclusion && (
        <div className="border-l-4 border-purple-500 pl-4">
          <h3 className="font-semibold text-fg mb-2 flex items-center gap-2">
            ⛓️ Execution Inclusion
          </h3>
          <div className="space-y-1 text-fg/80">
            {inclusion.block_number && (
              <div className="flex justify-between">
                <span className="text-fg/60">Block Number:</span>
                <span className="font-medium">{blockNumberToNumber(inclusion.block_number).toLocaleString()}</span>
              </div>
            )}
            {inclusion.block_hash && (
              <div className="flex justify-between">
                <span className="text-fg/60">Block Hash:</span>
                <span className="font-mono text-xs">{shortenHash(inclusion.block_hash)}</span>
              </div>
            )}
            {inclusion.transaction_index !== undefined && (
              <div className="flex justify-between">
                <span className="text-fg/60">Transaction Index:</span>
                <span>#{hexToNumber(inclusion.transaction_index)} {inclusion.total_transactions && `of ${inclusion.total_transactions} total`}</span>
              </div>
            )}
            {inclusion.timestamp && (
              <div className="flex justify-between">
                <span className="text-fg/60">Block Timestamp:</span>
                <span>{new Date(hexToNumber(inclusion.timestamp) * 1000).toLocaleString()}</span>
              </div>
            )}
            {inclusion.miner && (
              <div className="flex justify-between">
                <span className="text-fg/60">Block Producer:</span>
                <span className="font-mono text-xs">{shortenHash(inclusion.miner)}</span>
              </div>
            )}
            {inclusion.block_gas_used && inclusion.block_gas_limit && (
              <div className="flex justify-between">
                <span className="text-fg/60">Block Gas Usage:</span>
                <span>
                  {formatNumber(hexToNumber(inclusion.block_gas_used))} / {formatNumber(hexToNumber(inclusion.block_gas_limit))} 
          ...
          {inclusion.neighboring_transactions && inclusion.neighboring_transactions.length > 0 && (
            <div className="mt-3 pt-3 border-t border-line/10">
              <div className="text-fg/60 text-xs mb-2">Neighboring Transactions:</div>
      ...
      {pbsRelay ? (
        <div className="border-l-4 border-orange-500 pl-4">
          ...
          <div className="space-y-1 text-fg/80">
            {pbsRelay.builder_pubkey && (
              <div className="flex justify-between">
                <span className="text-fg/60">Builder:</span>
                <span className="font-mono text-xs">{shortenHash(pbsRelay.builder_pubkey)}</span>
              </div>
            )}
            {pbsRelay.proposer_pubkey && (
              <div className="flex justify-between">
                <span className="text-fg/60">Proposer:</span>
                <span className="font-mono text-xs">{shortenHash(pbsRelay.proposer_pubkey)}</span>
              </div>
            )}
            {pbsRelay.value && (
              <div className="flex justify-between">
                <span className="text-fg/60">Builder Payment:</span>
                <span className="font-medium text-green-400">{weiToEth(pbsRelay.value)} ETH</span>
              </div>
            )}
            {pbsRelay.relay && (
              <div className="flex justify-between">
                <span className="text-fg/60">Relay:</span>
                <span className="text-xs">{pbsRelay.relay}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="border-l-4 border-orange-500/50 pl-4">
          <h3 className="font-semibold text-fg/60 mb-2 flex items-center gap-2">
            🔄 Relay Bidtraces
          </h3>
          <div className="text-fg/60 text-xs">
            No MEV-Boost relay data found - this block may have been built locally (vanilla block)
          </div>
        </div>
      )}

      {/* Finality Check Section */}
      {beacon ? (
        <div className="border-l-4 border-cyan-500 pl-4">
          <h3 className="font-semibold text-fg mb-2 flex items-center gap-2">
            ✅ Finality Check (Beacon Checkpoints)
          </h3>
          <div className="space-y-2 text-fg/80">
            <div className="flex justify-between">
              <span className="text-fg/60">Consensus Slot:</span>
              <span className="font-medium">{beacon.slot?.toLocaleString() || 'N/A'}</span>
            </div>
            {beacon.finalized_epoch !== undefined && (
              <div className="flex justify-between">
                <span className="text-fg/60">Finalized Epoch:</span>
                <span className="font-medium">{beacon.finalized_epoch}</span>
              </div>
            )}
            {beacon.is_finalized !== undefined && (
              <div className="mt-2 p-2 rounded border" style={{
                backgroundColor: beacon.is_finalized ? 'rgba(34, 197, 94, 0.1)' : 'rgba(234, 179, 8, 0.1)',
                borderColor: beacon.is_finalized ? 'rgba(34, 197, 94, 0.3)' : 'rgba(234, 179, 8, 0.3)'
              }}>
                {beacon.is_finalized ? (
                  <div className="flex items-center gap-2">
                    <span className="text-green-400">✅</span>
                    <span className="font-semibold text-green-400">FINALIZED</span>
                    <span className="text-fg/80 text-xs">(Irreversible under normal conditions)</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2">
                    <span className="text-yellow-400">⏳</span>
                    <span className="font-semibold text-yellow-400">PENDING FINALITY</span>
                    <span className="text-fg/80 text-xs">(Waiting for checkpoint finalization)</span>
                  </div>
                )}
              </div>
            )}
            <div className="text-fg/60 text-xs mt-2">
              💡 Finality uses Casper-FFG checkpoints. Finalized blocks require ~$30B+ to reverse.
            </div>
          </div>
        </div>
      ) : !isPending && (
        <div className="border-l-4 border-cyan-500/50 pl-4">
          ...
          <div className="text-fg/60 text-xs">
            Finality data not available - beacon API may be unavailable
          </div>
        </div>
      )}
      ...
      <details className="mt-4">
        <summary className="cursor-pointer text-fg/60 text-xs hover:text-fg/80">
          Show raw JSON data
        </summary>
        <pre className="mt-2 text-xs bg-surface/60 p-3 rounded overflow-auto max-h-64">
